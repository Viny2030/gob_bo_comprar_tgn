...
Fuentes cubiertas:
  ...
  7. GEOREF — Normalización de domicilios y provincias (apis.datos.gob.ar/georef)
...
def obtener_comprar_api(
    ...
    try:
        # Primero intentar obtener URL actualizada desde el catálogo CKAN
        try:
            r_ckan = _get(
            ...
            for pkg in r_ckan.json().get("result", {}).get("results", []):
                for res in pkg.get("resources", []):
                    ...
                    if (url_res.endswith(".csv") and
                        ...
                        csv_url = url_res
                        print(f"  🔍 URL encontrada en CKAN: {url_res[:80]}")
                        break
...
def obtener_tgn_ejecucion_api(
    ...
    try:
        ...
        print(f"  ⚠️  TGN /credito no disponible: {e}")
        print("  💡 Intentando fallback directo de diario.py...")
        try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 7. GEOREF — Normalización de domicilios y georreferenciación
# ═══════════════════════════════════════════════════════════════════════════════
...
def normalizar_domicilio_georef(
    ...
    """
    Normaliza una dirección usando la API Georef oficial de Argentina.
...
def normalizar_domicilios_lote(
    ...
    Útil para enriquecer df_adjudicaciones con coordenadas de los proveedores.
    ...
    if df.empty or col_direccion not in df.columns:
        print(f"  ⚠️  Columna '{col_direccion}' no encontrada en el DataFrame")
        return df

    print(f"\n📡 Georef — normalizando {len(df)} domicilios...")
    resultados = []
    ...
    print(f"  ✅ {con_coords}/{len(df_out)} domicilios georreferenciados")
    return df_out
...
def validar_provincia_georef(nombre: str) -> dict:
    ...
    Útil para estandarizar provincias en el cruce de fuentes.
...
def obtener_todo_api(anio_comprar: int = 2020) -> dict:
    ...
    -------
    {
        ...
        "contrat_ocds":   df,
        "tgn_ejecucion":  df,
        "georef_test":    df,  # muestra de normalización Georef
    }
    """
    print("\n" + "═" * 55)
    ...
    try:
        # Test rápido Georef con domicilio conocido de ARCA
        test_geo = normalizar_domicilio_georef("Suipacha 767", "Ciudad Autónoma de Buenos Aires")
        resultados["georef_test"] = pd.DataFrame([test_geo])
    except Exception as e:
        print(f"  ❌ georef_test: {e}")
