...
======================
Script de verificación para las APIs oficiales.
Corre cada API, muestra resultado y diagnóstico de compatibilidad.
...
# mínimas que deben existir para que cruzar_fuentes() funcione correctamente.
SCHEMA_DIARIO = {
...
def _resultado(df: pd.DataFrame, columnas_muestra: list = None):
    if df is None or df.empty:
        print("  ⚠️  Sin datos — API no disponible o sin resultados hoy")
        return
...
def test_bora(texto: str = None):
    ...
    _titulo("1. BORA — Normativa API (sección tercera, hoy)")
...
def test_comprar(anio: int = 2020, organismo: str = None):
    ...
    # Años disponibles
    print(f"\n  📋 Años con datos en adjudicaciones: 2017, 2018, 2019, 2020")
...
def test_contrat():
    ...
    _titulo("3. CONTRAT.AR — Obra Pública OCDS (datos.gob.ar)")
...
def test_tgn(jurisdiccion: str = None):
    ...
    if not df.empty:
        ...
        if con_cuit_benef > 0:
            ...
            print(f"     ⚠️  cuit_beneficiario vacío — usando organismo_norm para cruce")
...
def test_cuit(cuit: str = "30-50000427-3"):
    ...
    _titulo(f"5. CUIT — Validación AFIP/SIPRO: {cuit}")
...
def test_cuit_lote(cuits: list = None):
    ...
    _titulo(f"5b. CUIT — Validación en lote ({len(cuits_test)} CUITs)")
...
def test_compatibilidad_con_diario():
    ...
    lógica que diario.py para detectar solapamientos.
    """
    ...
    if not df_comp.empty:
        ...
        print("  ⚠️  df_comprar: sin datos para verificar")
    ...
    if not df_tgn.empty:
        ...
        print("  ⚠️  df_tgn: sin datos para verificar")
    ...
    if not df_comp.empty and not df_tgn.empty:
        ...
        if cruces > 0:
            ...
            print("  ⚠️  Sin cruces en el mini-test (normal si son datos de años distintos)")
    else:
        print("  ⚠️  No hay datos suficientes para el mini-cruce")

    # Verificar que las funciones originales de diario.py aún funcionan
    ...
    try:
        ...
        for f in funcs:
            assert hasattr(diario, f), f"⚠️  Falta función {f} en diario.py"
...
def test_enriquecimiento_reporte():
    """
    Demo: cómo usar apis_oficiales para enriquecer un reporte
    ...
    if not reportes:
        print("  ⚠️  No hay reportes en data/ — ejecutá diario.py primero")
        return
    ...
    try:
        ...
        if "cuit_proveedor" not in df.columns:
            print("  ⚠️  Columna 'cuit_proveedor' no encontrada — usando datos demo")
        ...
        if not cuits_test:
            print("  ⚠️  Sin CUITs válidos en el reporte")
            return
...
def main():
    ...
    parser.add_argument(
        ...
        help="Correr solo una API/test específico",
    )
